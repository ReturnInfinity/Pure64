; =============================================================================
; BMFLFS -- BareMetal FLoppy File System Utility for x86-64 systems
...
fdc_recalib:
	...
	call fdc_sendbyte
	mov	byte [res_C], 0		; ovde premestio
...
fdd_rw:
	...
	mov	rcx, [dma_buff]	; load addres of buf for transfer
...
; -----------------------------------------------------------------------------
; This procedure is releated with pure64.asm file from Pure64 repo
show_status:
	xor ebx, ebx
	mov eax, 0x00ffffff
	sub eax, [debug_box]
...
; cylinders from both sides: 3 * 0x4800 = 0xd800 or 55296 bytes * 26 repetes
...
debug_box	db	8		; Releated with Pure64
dma_buff	dd	DMA_BUFF
mem_buff	dd	MEM_BUFF
box_color	dd	0x00F7CA54	; Return Infinity Yellow/Orange from Pure64
